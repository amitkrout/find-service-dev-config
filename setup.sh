...
sed $SED_OPTIONS "s|PULL_SECRET_NAME|${PULL_SECRET_NAME}|g" pipelines/01-serviceaccount/serviceaccount.yaml deploy/*.yaml
...
sed $SED_OPTIONS "s|GITHUB_REPO|${GITHUB_REPO}|g" pipelines/06-eventlisteners/cicd-event-listener.yaml
sed $SED_OPTIONS "s|GITHUB_STAGE_REPO|${GITHUB_STAGE_REPO}|g" pipelines/06-eventlisteners/cicd-event-listener.yaml pipelines/argocd/argo-app.yaml
